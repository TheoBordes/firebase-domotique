...
void loop() {
  ...
  if (bNewInt & !timer4 & !button1) {
    ...
    clearInt(mfrc522);
    mfrc522.PICC_HaltA();
    if (name == "53 7C C8 A9" & !state_led & !timer4 & !button1){relais_once(); led_time = currentTime;}
    name = "";
    bNewInt = false;
 ...
 if ( state_led & (currentTime - led_time > 10000) & !timer4 & !button1 ){relais_once();}
